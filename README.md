...
| <img src="docs/icons/function_generator.png" width="32"> **Function Generator** | Time-varying voltage source. Empty-hand right-click cycles sine/square/triangle presets (waveform shape only - see Voltage/Frequency Module below for amplitude and frequency, both relayed in from a neighboring module). Shift+right-click opens its own value editor for **Phase** (0–360°, applies to all three waveform shapes, not just sine) - the one parameter set directly on the generator itself rather than through a module. Same redstone-activation behavior as the Power Supply. Defaults to 5V/1Hz/0° with no modules attached. |
